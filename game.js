...
const state = {
  ...
  aiFunds:createAiFunds(), institutionActivity:[]
};
...
function executeTrade() {
  ...
  executeMarketTrade(company,state.side,qty);
}
...
const events = [
  ["wins a major government contract",.10],["reports stronger-than-expected demand",.07],["launches a promising new product",.06],
  ["faces a regulatory investigation",-.11],["warns of rising input costs",-.07],["loses a key customer",-.09],
  ["announces a share buyback",.05],["issues new shares to fund expansion",-.04]
];
...
function advanceDay(renderAfter=true) {
  ...
  if (Math.random()<.48) {
    ...
    c.price=Math.max(2,c.price*(1+event[1]));
    state.news.unshift({day:state.day,text:`${c.name} ${event[0]}.`,impact:event[1],ticker:c.ticker});
